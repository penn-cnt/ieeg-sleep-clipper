...
print(f"Test date of this run: {test_date}. Test time: {test_time}")

# for each vigilance state
window_length = 15 # seconds
# get events for this run
run_events = vigilance_events[np.char.startswith(vigilance_events[:,0],test_date)]
...
for vigilance_state in ['wake', 'N1', 'N2', 'N3', 'REM']:
    ...
    onset = round(time_to_seconds(random_event[0].split("T")[1]) - time_to_seconds(test_time))
    print(random_event, f"onset time (s): {onset}")
